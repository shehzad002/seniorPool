...
/**
 * @title goflamingo's SeniorPool contract
 * @notice Main entry point for senior LPs (a.k.a. capital providers)
 *  Automatically invests across borrower pools using an adjustable strategy.
 * @author goflamingo
 */
contract SeniorPool is BaseUpgradeablePausable, ISeniorPool {
  goflamingoConfig public config;
  using ConfigHelper for goflamingoConfig;
  ...
  event goflamingoConfigUpdated(address indexed who, address configAddress);

  function initialize(address owner, goflamingoConfig _config) public initializer {
  ...
  /**
   * @notice Migrates to a new goflamingo config address
   */
  function updategoflamingoConfig() external onlyAdmin {
    config = goflamingoConfig(config.configAddress());
    emit goflamingoConfigUpdated(msg.sender, address(config));
  }
